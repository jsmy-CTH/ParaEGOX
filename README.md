...
governance, contract, and Agent-worker tests passed on the Mac without invoking Rust. The r19 source
then produced a checksum-verified native x86_64 Mac executable in the `macos-cli-artifact` workflow;
that downloaded executable completed one real offline Echo TUI round trip and clean shutdown on the
Mac. These results are not the full workspace test/clippy/doc gates, fresh Ubuntu CI, or production
evidence. A credentialed external DeepSeek smoke is still pending, so the configured DeepSeek path
must not yet be described as externally validated or production ready. For an offline substrate
